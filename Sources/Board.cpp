...
Board::Board(QColor bgColor) {
    ...
    b->addParam(new Parameter("de", ValueType::INT));
    b->addParam(new Parameter("ate", ValueType::INT));

    Brick* b1 = new FunctionBrick("Escreva no Serial", QColor(0, 128, 0));
    b1->addParam(new Parameter(ValueType::STRING));
    ((StatementBrick*)b)->addBrick(b1);

    b1 = new FunctionBrick("AnalogWrite", QColor(0, 128, 0));
    b1->addParam(new Parameter("Porta:", ValueType::STRING));
    b1->addParam(new Parameter("Valor:", ValueType::STRING));
    ((StatementBrick*)b)->addBrick(b1);

    b1 = new FunctionBrick("AnalogRead", QColor(0, 128, 0));
    Parameter* p = new Parameter("Porta:", ValueType::INT);
    p->setValue(new Value(123));
    b1->addParam(p);
    ((StatementBrick*)b)->addBrick(b1);
