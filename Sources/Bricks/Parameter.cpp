...
Parameter::Parameter(QString textBefore, QString textAfter, ValueType type) : Parameter(textBefore, type) {
...
Parameter::Parameter(QString textBefore, ValueType type) : Parameter(type) {
    this->textBefore = textBefore;
}

QSize Parameter::size(QFont font) {
    int width = value != NULL && !value->isEmpty() ? value->size().width() : EMPTY_VALUE_WIDTH;
    ...
    int height = value != NULL && !value->isEmpty() ? value->size().height() : EMPTY_VALUE_HEIGHT;
